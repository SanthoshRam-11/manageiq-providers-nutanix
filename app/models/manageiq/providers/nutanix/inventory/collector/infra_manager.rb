class ManageIQ::Providers::Nutanix::Inventory::Collector::InfraManager < ManageIQ::Providers::Nutanix::Inventory::Collector
  ...
  def vms
    @vms ||= begin
      ...
      list = vms_api.list_vms_0
      list.data.map do |vm_summary|
        begin
          # Fetch full VM
          resp = vms_api.get_vm_by_id_0(vm_summary.ext_id)
          resp.data  # ✅ Extract actual VM object
        rescue => e
          $log.warn("Failed to get full VM details for #{vm_summary.ext_id}: #{e}")
          nil
        end
      end.compact
    end
